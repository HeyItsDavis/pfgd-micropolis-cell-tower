...
public class BudgetDialog extends JDialog
{
	...
	JLabel cellFundRequest = new JLabel();
	JLabel cellFundAlloc = new JLabel();
	JSlider cellFundEntry;
	...
	private void applyChange()
	{
		...
		int newCellPct = ((Number) cellFundEntry.getValue()).intValue();
		...
		engine.cellServicePercent = (double)newCellPct / 100.0;
	...
	private void loadBudgetNumbers(boolean updateEntries)
	{
		...
		cellFundEntry.setValue((int)Math.round(b.cellServicePercent*100.0));
		}
		...
		cellFundRequest.setText(formatFunds(b.cellServiceRequest));
		cellFundAlloc.setText(formatFunds(b.cellServiceFunded));
	}
	...
	public BudgetDialog(Window owner, Micropolis engine)
	{
		...
		this.origCellPct = engine.cellServicePercent;
		...
		cellFundEntry = new JSlider(JSlider.HORIZONTAL, 0, 100, 100);
		adjustSliderSize(cellFundEntry);
		...
		cellFundEntry.addChangeListener(change);
	...
	private JComponent makeFundingRatesPane()
	{
		...
		fundingRatesPane.add(new JLabel(strings.getString("budgetdlg.cell_fund")), c0);
		fundingRatesPane.add(cellFundEntry, c1);
		fundingRatesPane.add(cellFundRequest, c2);
		fundingRatesPane.add(cellFundAlloc, c3);
	...
	private void onResetClicked()
	{
		...
		engine.cellServicePercent = this.origCellPct;
		loadBudgetNumbers(true);
	}
